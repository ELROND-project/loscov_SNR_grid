    ...
    for a1 in range(Nbin):
        ...
        if b1 == b2:
            ...
            if not approx:
                ...
                pp_err2 += err1**2 + (4*integral2square*err2)**2
            ...
            pp_err2 = pp_err2 * (Omegatot / ( 2 * Omegas[a1] * Nlens * Ngal1 ))**2
    ...
    for a1 in range(Nbin):
        ...
        if b1 == b2:
            ...
            if not approx:
                ...
                mm_err2 += err1**2 + (4*integral2square*err2)**2
            
            integral /= Omegas[a1]
            mm_err /= (Omegas[a1])**2
        
            scov_mm[a1, a1] = integral * Omegatot / ( 2 * Omegas[a1] * Nlens * Ngal1 )
            mm_err2 = mm_err2 * (Omegatot / ( 2 * Omegas[a1] * Nlens * Ngal1 ))**2
        ...
        for a2 in range(a1, Nbin):
            ...
            test_err(mm_err2, scov_pp[a1, a2], f'LeLe scov mm redshifts {b1, b2} angular bins {a1, a2}')
...
####################################### Plus - minus block ###########################################
    ...
    for a1 in range(Nbin):  
        ...
        if b1 == b2 and not approx:
            ...
            pm_err = err * (2 * Omegatot / ( Omegas[a1] * Nlens * Ngal1 ))**2
...
####################################### Minus - plus block ###########################################
    ...
    for a1 in range(Nbin):
        ...
        if b1 == b2 and not approx:
            ...
            integral2, err = monte_carlo_integrate(xi2_plus_integrand, ranges, 2e1)
            integral3, err = monte_carlo_integrate(xi2_minus_integrand, ranges, 3e1)
            integral -= 2*integral2*integral3
            integral /= Omegas[a1]

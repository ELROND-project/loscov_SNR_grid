...
class Distributions:
    ...
    def __init__(self, Nlens=Nlens, sky_coverage=sky_coverage, Nbina=Nbina, Thetamax=Thetamax, binscheme=None):
        """
        Arguments:
        ...
        self.Nlens = Nlens  #the number of lenses
        self.Omegatot = sky_coverage * (np.pi / 180)**2 # in rad2
        self.lens_density = Nlens / self.Omegatot #the density of lenses in an angular bin
        
        # Binning
        self.Nbina = Nbina #the number of angular bins
...
def generate_binned_correlation(distributions, cov_matrix, b1, b2):
    ...
    correlation_mapping = {
        "LLLL": lambda: {
            "plus_correlation": distributions.compute_binned_correlation(xi2_LOS_plus_intp),
            "minus_correlation": distributions.compute_binned_correlation(xi2_LOS_minus_intp),
        },
        "LeLe": lambda: {
            "plus_correlation": distributions.compute_binned_correlation(xi2_LOS_eps_minus_intp[b1]),
            "minus_correlation": distributions.compute_binned_correlation(xi2_LOS_eps_minus_intp[b1]),
        } if b1 == b2 else None,
        "LpLp": lambda: {
            "correlation": distributions.compute_binned_correlation(xi2_LOS_d_intp[b1]),
        } if b1 == b2 else None
    }

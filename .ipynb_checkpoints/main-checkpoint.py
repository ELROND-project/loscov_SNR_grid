...
def compute_covariance_piece(args):
    ...
    if cov_type == "ncov":
        result = func(distributions, sigma_noise, sigma_shape, b1, b2)
    else:
        result = func(distributions, b1, b2)
...
def main():
    ...
    with Pool(processes=cpu_count()) as pool:
        ...
        # Compute covariance matrices in parallel
        results = pool.map(compute_covariance_piece, cov_args)

    # Save covariance matrices after processing
    for b1, b2, cov_matrix, cov_type, data in results:
        if data is None:
